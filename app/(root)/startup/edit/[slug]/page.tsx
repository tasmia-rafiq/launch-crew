...
import { STARTUP_BY_ID_QUERY, STARTUP_BY_SLUG_QUERY } from "@/sanity/lib/queries";
import {
  FileTextIcon,
  FoldersIcon,
  ImageIcon,
...
const page = async ({ params }: { params: Promise<{ slug: string }> }) => {
  ...
  const postDetails = await client.fetch(STARTUP_BY_SLUG_QUERY(session?.id), { slug }, { useCdn: false });

  // Extract id from postDetails, fallback to undefined if not present
  const id = postDetails?._id;

  return (
    <>
      <section className="pink_container mt-22.5 !min-h-[130px]">
        <h1 className="heading">Edit Your Startup Idea</h1>
      </section>

      <section className="py-10 px-6 flex flex-row justify-between items-start gap-6">
        <div className="w-[40%] border-1 border-[#cecece] rounded-lg p-5 prism-bg">
            ...
            <li className="flex gap-2 items-start">
              ...
              <div className="flex flex-col items-start gap-1">
                ...
                <p className="text-14-normal !text-black-100 leading-5">
                  ...
                  glance, keep it punchy and clear. Example:{" "}
                  <b>
                    “An AI-driven tool that helps remote teams monitor mental
                    wellness in real-time.”
                  </b>
                </p>
              </div>
            </li>
        ...
        <StartupForm id={id} postDetails={postDetails} />
      </section>
    </>
  );
};
